  ...
  return <div className="relative mx-auto max-w-[1160px] px-5 pt-6 pb-10 md:px-10 md:pt-10">
    ...
    <div className="mt-4 grid items-center gap-7 lg:grid-cols-[1.1fr_1fr] lg:gap-12">
      <div className="relative h-[min(32dvh,300px)] overflow-hidden rounded-xl border border-white/10 bg-ink-800 lg:h-auto lg:aspect-[4/3]">
        <Image src={story.heroScene ?? story.cover} alt={`Cover scene from ${story.title}`} fill sizes="(min-width: 1024px) 52vw, 92vw" loading="eager" fetchPriority="high" className="object-contain" />
      ...
      <div>
        ...
        <h1 className="mt-3 font-display text-[clamp(1.9rem,6vw,3rem)] leading-[1.1] font-medium text-mist-100">{story.title}</h1>
        <p className="mt-4 max-w-[52ch] text-[15px] leading-relaxed text-mist-300">{story.description}</p>
